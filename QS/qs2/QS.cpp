...
using namespace std;
	...
	void QS::swap(int first, int second) {
		int temp = qsArray[first];
		qsArray[first] = qsArray[second];
		qsArray[second] = temp;
	}
	...
	*/
	int QS::medianOfThree(int left, int right) {
		...
		if (left >= right || left < 0 || right > (arrSize - 1)) {
			return -1;
		}
	...
	*/
	int QS::partition(int left, int right, int pivotIndex) {
		...
		do {
			...
			while ((up != right) && !(qsArray[left] < qsArray[up])) {
				up++;
			}
	...
	*/
	bool QS::addToArray(int value) {
		...
		else if (arrayAt < maxCap) {
			qsArray[arrayAt] = value;
			arrSize = arrayAt + 1;
			arrayAt++;
			return true;
		}
